...
public class BoardActionManager : MonoBehaviour
{
    ...
    private void SwapItems(GameObject draggedObj, GameObject targetObj, Transform sourceAnchor, Vector3 sourcePos)
    {
        ...
        {
            targetObj.transform.localPosition = sourcePos;
        }
        ...
        DOVirtual.DelayedCall(slideTime, () =>
        {
            if (sourceShelf != null)
            {
                for (int i = 0; i < 3; i++)
                {
                    if (sourceShelf.slots[i] == targetObj) sourceShelf.PlayDropGlow(i);
                }
            }
            ...
            if (targetShelf != null)
            {
                for (int i = 0; i < 3; i++)
                {
                    if (targetShelf.slots[i] == draggedObj) targetShelf.PlayDropGlow(i);
                }
            }
